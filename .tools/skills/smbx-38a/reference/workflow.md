...
python .tools/skills/smbx-38a/scripts/lvl_query.py "<lvl>" --kind N --filter id=283

# 4. 拿事件清单
python .tools/skills/smbx-38a/scripts/lvl_query.py "<lvl>" --kind E --fields name
```
...
python .tools/skills/smbx-38a/scripts/lvl_scripts.py extract "<lvl>" -o ".cache/scripts/level1"

# 步骤 B：用普通 read_file/replace_in_file 工具修改 <dir>/<scriptname>.tea

# 步骤 C：回写到新 lvl（默认输出 <lvl>.modified.lvl）
python .tools/skills/smbx-38a/scripts/lvl_scripts.py inject "<lvl>" \
       -i ".cache/scripts/level1" -o "<lvl>.modified.lvl"

# 步骤 D：round-trip 校验（解析新文件，确保结构无破坏）
python .tools/skills/smbx-38a/scripts/lvl_parse.py "<lvl>.modified.lvl" --summary
```
...
每次改完都建议：
```bash
python .tools/skills/smbx-38a/scripts/lvl_parse.py "<out.lvl>" --summary
```
...
python .tools/skills/smbx-38a/scripts/engine_control.py state --camera
python .tools/skills/smbx-38a/scripts/engine_control.py state --var "myVar"

# 5. 显示一个 messagebox 检查脚本是否还活着
python .tools/skills/smbx-38a/scripts/engine_control.py message --text "ping"

# 6. 修改了 lvl 后，重载
python .tools/skills/smbx-38a/scripts/engine_control.py reload \
